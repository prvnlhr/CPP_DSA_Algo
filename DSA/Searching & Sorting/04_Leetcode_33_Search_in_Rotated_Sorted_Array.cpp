...
int binHelper(vector<int> &nums, int s, int e, int x)
{
    if (s > e)
    {
        return -1;
    ...
    {
        if (nums[s] <= x && x <= nums[mid])
        {
            return binHelper(nums, s, mid - 1, x);
        }
        else
        {
            return binHelper(nums, mid + 1, e, x);
        }
    }
    else if (nums[mid] < nums[e]) //> right half sorted
    {
        if (nums[mid] <= x && x <= nums[e])
        {
            return binHelper(nums, mid + 1, e, x);
        }
        else
