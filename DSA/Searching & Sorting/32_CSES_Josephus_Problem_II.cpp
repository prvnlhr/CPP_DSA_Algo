...
// |> https://www.geeksforgeeks.org/order-of-removal-in-josephus-problem-in-on-logn/

void solve1() //|> Using vector solution , didnt work
{
    int n, k;
    cin >> n >> k;
    ...
    */
    ...
    while (childrens.size() > 1)
    {
        // debug(childrens.size());
        debug(childrens, pos, childrens.size());
        for (int i = 0; i < childrens.size(); i++)
        {
            ...
            if (i == pos)
            {
                debug(i);
                cout << childrens[i] << " ";
                pos = (pos + k + 1) % childrens.size();
            }
            else
            {
                debug(i, childrens[i], not_taken);
                not_taken.push_back(childrens[i]);
            }
        }
        debug(not_taken);
        if (not_taken.size() == 0)
        ...
        childrens.clear();
        // debug(pos, "clear");
        for (int ele : not_taken)
        {
            childrens.push_back(ele);
        }
        if (pos >= childrens.size())
        {
            pos = pos % childrens.size();
        }

        // debug(pos, childrens, childrens.size());
        not_taken.clear();
    }
    // debug(childrens);
    cout << '\n';
...
void solve() //|> using order_set in c++ STL. refer GFG Link above
{
    int n, k;
    cin >> n >> k;

    tree<int, null_type, less<int>, rb_tree_tag, tree_order_statistics_node_update> children;
