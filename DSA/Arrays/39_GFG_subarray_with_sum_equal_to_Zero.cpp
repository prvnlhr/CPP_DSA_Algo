...
vector<pair<int, int>> subarraySum(vector<int> arr, int n)
{
    ...
    for (int i = 0; i < n; i++)
    {
        ...
        {
            vector<int> allIndices = mpp[currSum];
            for (auto indx : allIndices)
            {
                res.push_back({indx + 1, i});
            }
        }

        mpp[currSum].push_back(i);
    }
    return res;
}
