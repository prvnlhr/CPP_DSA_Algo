...
void solve()
{
    ...
    vector<vector<long long>> dp(2, vector<long long>(10000001, 0));

    // i = 1,  type -> 0  [] [] , 2 Blocks, each of 1x1
    dp[0][1] = 1;
    dp[1][1] = 1;

    // i = 2,  type -> 1  [   ],   1 Block of 2x1
    dp[0][2] = 5;
    dp[1][2] = 3;

    for (int i = 3; i <= 1000000; i++)
    {
        long long type0 = dp[0][i - 1] * 5;
        long long type1 = dp[1][i - 1] * 3;

        long long b1 = (((4 * type0) / 5) % MOD + ((1 * type1) / 3) % MOD) % MOD;
        long long b2 = (((1 * type0) / 5) % MOD + ((2 * type1) / 3) % MOD) % MOD;
    ...
    while (t--)
    {
        int n;
        cin >> n;
        cout << ((dp[0][n] + dp[1][n]) % MOD) % MOD << endl;
    }
}
