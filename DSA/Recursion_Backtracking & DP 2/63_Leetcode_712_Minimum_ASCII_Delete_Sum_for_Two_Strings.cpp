...
int dead_end_sum(const string &s, int i)
{
    int sum = 0;
    while (i >= 0)
    {
...
int recursive(string &word1, string &word2, int m, int n)
{
    if (m == 0)
    {
        return dead_end_sum(word2, n - 1);
    }
    if (n == 0)
    {
        return dead_end_sum(word1, m - 1);
    }
    ...
    {
        int a = int(word1[m - 1]) + recursive(word1, word2, m - 1, n);
        int b = int(word2[n - 1]) + recursive(word1, word2, m, n - 1);
        sum = min(a, b);
    }
    else
    {
        sum = recursive(word1, word2, m - 1, n - 1);
    }
