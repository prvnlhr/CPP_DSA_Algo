...
void backtrack(int n, int pos, string curr) // O(n)
{
    if (curr.size() == n)
    {
        cnt++;
    ...
    for (int i = pos; i < vov.size(); i++) // O(5)
    {
        curr += vov[i];
        backtrack(n, i, curr);
        curr.pop_back();
...
int recursiveMemo(int n, int pos, vector<vector<int>> &dp)
{
    if (n == 0)
    {
        return 1;
...
{
    vector<vector<int>> dp(n + 1, vector<int>(vov.size(), -1));
    return recursiveMemo(n, 0, dp);
}
