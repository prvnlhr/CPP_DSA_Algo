...
int numRollsToTargetDPMemo(int diceRem, int n, int k, int target, vector<vector<int>> &dp)
{

    debug(target, diceRem);
    if (diceRem == 0)
    {
        if (target == 0)
        {
    ...
    if (dp[diceRem][target] != -1)
    {
        return dp[diceRem][target];
    }
    ...
    for (int i = 1; i <= k; i++)
    {
        if (i <= target)
        {
            res = (res + numRollsToTargetDPMemo(diceRem - 1, n, k, target - i, dp)) % MOD;
        }
    }
    return dp[diceRem][target] = res;
}
...
int numRollsToTarget(int n, int k, int target)
{
    ...
    return numRollsToTargetDPMemo(n, n, k, target, dp);
}
void solve()
{
    int n, k, target;
