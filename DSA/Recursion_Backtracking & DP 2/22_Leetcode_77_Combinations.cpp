...
// #define FOR(i, idx, end) for (int i = idx; i < end; i++)
#define FOR(i, begin, end) for (__typeof(end) i = (begin) - ((begin) > (end)); i != (end) - ((begin) > (end)); i += 1 - 2 * ((begin) > (end)))
#define RFOR(i, idx, end) for (int i = end; i >= idx; i--)
...
void helper(int idx, int n, int k, vector<int> &comb)
{
    if (comb.size() == k)
    {
        res.push_back(comb);
...
#endif
    auto idx1 = high_resolution_clock::now();
    solve();
    auto stop1 = high_resolution_clock::now();
    auto duration = duration_cast<microseconds>(stop1 - idx1);
