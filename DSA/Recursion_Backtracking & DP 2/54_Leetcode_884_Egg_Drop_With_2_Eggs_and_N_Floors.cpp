...
int recursive(int eggs, int floors)
{
    ...
    {
        int a = recursive(eggs - 1, currFloor - 1);
        int b = recursive(eggs, floors - currFloor);
        int maxi = 1 + max(a, b);
        minMoves = min(minMoves, maxi);
    }
    return minMoves;
}

vector<vector<int>> dp;
int recursiveMemo(int floors, int eggs)
{
    ...
    for (int currFloor = 1; currFloor <= floors; currFloor++)
    {
        int willBreak = recursiveMemo(currFloor - 1, eggs - 1);
        int notBreak = recursiveMemo(floors - currFloor, eggs);
        ans = min(ans, max(willBreak, notBreak));
    }
    return dp[floors][eggs] = ans + 1;
}

int twoEggDrop(int n)
{
    dp = vector<vector<int>>(n + 1, vector<int>(3, 0));
    return recursiveMemo(n, 2);
}
