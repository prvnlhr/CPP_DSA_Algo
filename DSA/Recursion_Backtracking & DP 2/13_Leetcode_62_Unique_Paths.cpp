...
void helper(int m, int n, int i, int j, set<string> &path, string currPath)
{
    if (i == m - 1 && j == n - 1)
    {
        path.insert(currPath);
    }
    else if (i >= m || j >= n)
    {
        return;
    }
    helper(m, n, i + 1, j, path, currPath + 'R');
    helper(m, n, i, j + 1, path, currPath + 'D');
}
int helperBetter(int m, int n, int i, int j, vector<vector<int>> &dp)
{
    if (i == m - 1 && j == n - 1)
    {
        return 1;
    ...
    int l = helperBetter(m, n, i + 1, j, dp);
    int r = helperBetter(m, n, i, j + 1, dp);
    dp[i][j] = l + r;
    return l + r;
}
