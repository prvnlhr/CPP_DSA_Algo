...
int numofArrSol(vector<int> &arr, int n, int m, int indx, int num)
{
    ...
    if (arr[indx] == 0)
    {
        if (indx == 0 && abs(arr[indx - 1] - arr[indx]) <= 1 && abs(arr[indx] - arr[indx + 1]) <= 1)
        {
            a = numofArrSol(arr, n, m, indx + 1, num + 1);
        }
        else if (indx == n - 1 && abs(arr[indx - 1] - arr[indx]) <= 1)
        {
            a = numofArrSol(arr, n, m, indx + 1, num + 1);
        }
        else
        ...
        b = numofArrSol(arr, n, m, indx, num + 1);

        return a + b;
    }
    else
    {
        return numofArrSol(arr, n, m, indx + 1, num);
    }
}

int numofArr(vector<int> &arr, int n, int m)
{
    int res = 0;
    for (int i = 1; i <= m; i++)
    {
        res += numofArrSol(arr, n, m, 0, i);
    }
    return res;
}
