...
ll recursive(ll arr[], ll start, ll end)
{
    if (start > end)
    {
        return 0;
    ...
    ll op1 = arr[start] + min(recursive(arr, start + 2, end), recursive(arr, start + 1, end - 1));
    ll op2 = arr[end] + min(recursive(arr, start + 1, end - 1), recursive(arr, start, end - 2));
