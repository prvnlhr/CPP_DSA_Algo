...
void helper(int curr_digit, int n, int k, vector<int> &res)
{

    if (cntDigits(curr_digit) == n)
    {
        res.push_back(curr_digit);
        return;
    }

    for (int i = 0; i < 10; i++)
    {
        int lastDigit = curr_digit % 10;
        if (abs(lastDigit - i) == k)
        {
            int newDigt = curr_digit * 10 + i;
            helper(newDigt, n, k, res);
        }
    }
}
...
void solve()
{
    ...
    auto res = numsSameConsecDiff(n, k);
    debug(res);
}
