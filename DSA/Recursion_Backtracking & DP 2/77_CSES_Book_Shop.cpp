...
int maxPages(int n, int x, vector<int> &prices, vector<int> &pages, vector<vector<int>> &dp)
{
    if (n == 0 || x <= 0)
    {
        return 0;
    ...
    if (prices[n - 1] <= x)
    {
        int takeBook = pages[n - 1] + maxPages(n - 1, x - prices[n - 1], prices, pages, dp);
        int skipBook = maxPages(n - 1, x, prices, pages, dp);
        result = max(takeBook, skipBook);
    }
    else
    {
        result = maxPages(n - 1, x, prices, pages, dp);
    }
...
int maxPagesDP(int n, int x, vector<int> &prices, vector<int> &pages)
{
...
void solve()
{
    ...
    // int res = maxPages(n, x, prices, pages, dp);
    int res = maxPagesDP(n, x, prices, pages);
    debug(res);
}
