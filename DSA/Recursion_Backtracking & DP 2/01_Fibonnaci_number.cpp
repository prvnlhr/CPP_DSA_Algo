...
int fibRecursiveDp(int n, vector<int> &dp)
{
    if (n == 0 || n == 1)
    {
        return n;
    ...
    {
        int currAns = fibRecursiveDp(n - 1, dp) + fibRecursiveDp(n - 2, dp);
        dp[n] = currAns;
        return dp[n];
    }
}
...
void solve()
{
    ...
    dp[0] = 0;
    dp[1] = 1;
    cout << fibRecursiveDp(n, dp) << endl;

    cout << iterativeDp(n) << endl;
}
