...
int findLongestChain(vector<vector<int>> &pairs)
{
    ...
    for (int i = 0; i < pairs.size(); i++)
    {
        ...
        if (prevEnd == INT_MIN || prevEnd < currStart) //|> prev interval is not-overlapping with curr Interval
        {
            prevEnd = currEnd;
            len++;
        }
        else if (prevEnd > currEnd) // |> if prev interval end is larger then curr interval
        {
            prevEnd = currEnd;
        }
    }
...
void solve()
{
    ...
    // vector<vector<int>> pairs{{4, 5}, {2, 3}, {2, 5}};
    // vector<vector<int>> ;
    // vector<vector<int>> pairs{{2, 3}, {8, 9}, {3, 6}, {6, 8}, {2, 5}, {6, 7}, {4, 5}};
