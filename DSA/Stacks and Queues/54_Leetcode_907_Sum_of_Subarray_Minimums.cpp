...
int sumSubarrayMins(const vector<int> &arr)
{
    ...
    vector<int> leftSmallestCnt(n);
    vector<int> rightSmallestCnt(n);
    ...
    for (int i = 0; i < n; ++i)
    {
        ...
        leftSmallestCnt[i] = st.empty() ? i + 1 : i - st.top();
        st.push(i);
    }
    ...
    for (int i = n - 1; i >= 0; --i)
    {
        ...
        rightSmallestCnt[i] = st.empty() ? n - i : st.top() - i;
        st.push(i);
    }

    debug(leftSmallestCnt);
    debug(rightSmallestCnt);

    int res = 0;
    for (int i = 0; i < n; ++i)
    {
        res = (res + (long long)arr[i] * (long long)leftSmallestCnt[i] * (long long)rightSmallestCnt[i]) % MOD;
    }
