...
void moveUntil(int x, int y, vector<vector<int>> &maze, vector<vector<int>> &visited, vector<string> &res, string path)
{
    if (x == N - 1 && y == N - 1)
    {
        res.push_back(path);
    ...
    for (int i = 0; i < 4; i++)
    {
        ...
        if (isSafe(nextX, nextY, visited, maze))
        {
...
void solve()
{
    ...
    if (maze[0][0] == 1)
    {
        moveUntil(0, 0, maze, visited, res, "");
    }
