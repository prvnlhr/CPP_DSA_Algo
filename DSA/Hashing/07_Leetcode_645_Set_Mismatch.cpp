...
typedef unordered_map<int, int> unmpint;
...
vector<int> findErrorNums(vector<int> &nums)
{
    ...
    /*
        > unordered_map has an average O(1) lookup, but in worst cases (due to collisions), it can degrade to O(N).
        > Using a fixed-size array (O(1) space) is faster and memory-efficient.
    */
    ...
    int missingNum = expectedSum - (actualSum - repeatedNum);
