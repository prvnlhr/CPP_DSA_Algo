...
int findShortestSubArray(vector<int> &nums)
{
    ...
    unordered_map<int, vector<int>> freqMpp;
    int maxFreq = INT_MIN;

    for (int index = 0; index < arrSize; index++)
    {
        int num = nums[index];
        if (freqMpp.find(num) != freqMpp.end())
        {
            auto ele = freqMpp[num];
            int newFreq = ele[0] + 1;
            int newStart = min(ele[1], index);
            int newEnd = max(ele[2], index);
            freqMpp[num] = {newFreq, newStart, newEnd};
            maxFreq = max(maxFreq, newFreq);
        }
        else
        {
            freqMpp[num] = {1, index, index};
            maxFreq = max(maxFreq, 1);
        };
    }
    int res = INT_MAX;
    for (auto &ele : freqMpp)
    {
        auto vec = ele.second;
        if (vec[0] == maxFreq)
        {
            res = min(res, abs(vec[2] - vec[1]) + 1);
        }
    }
    return res;
}

// improvised for cleaner code
int findShortestSubArray(vector<int> &nums)
{
    int arrSize = nums.size();
    unordered_map<int, vector<int>> freqMpp;
    ...
    for (int index = 0; index < arrSize; index++)
    {
        ...
        maxFreq = max(maxFreq, freqMpp[num][0]);
    }

    for (auto &[num, vec] : freqMpp)
    {
        if (vec[0] == maxFreq)
        {
