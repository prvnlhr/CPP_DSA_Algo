...
bool containsNearbyDuplicate(vector<int> &nums, int k)
{
    ...
    while (winStart < n)
    {
        debug(winStart, winEnd, nums[winStart], nums[winEnd]);

        if (winEnd - winStart < k && winEnd != winStart)
        {
            if (nums[winStart] == nums[winEnd] == k)
            {
                return true;
            }
            else
        ...
        else if ((winEnd - winStart == k) || winEnd >= n)
        {
...
bool containsNearbyDuplicate1(vector<int> &nums, int k)
{
    ...
    for (int j = 0; j < n; j++)
    {
        ...
        if (mpp.find(ele) != mpp.end() && (abs(j - mpp[ele])) <= k)
        {
            return true;
        }
        mpp[ele] = j;
