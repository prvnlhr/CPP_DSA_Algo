...
string repeatLimitedString1(string s, int repeatLimit)
{
    unordered_map<char, int> mpp;
    for (auto ch : s)
    {
    ...
    while (!pq.empty())
    {
        ...
        for (int i = 0; i < min(freq1, repeatLimit); i++)
        {
            res += ch1;
            cnt--;
        }
...
void solve()
{
    ...
    int repeatLimit;
    cin >> repeatLimit;
    string res = repeatLimitedString1(s, repeatLimit);
    debug(res);
}
