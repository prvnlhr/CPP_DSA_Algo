...
int eatenApples(vector<int> &apples, vector<int> &days)
{
    ...
    while (currDay < days.size() || !pq.empty())
    {
        if (currDay < days.size() && apples[currDay] > 0)
        {
            pq.push({currDay + days[currDay], apples[currDay]});
        }
        while (!pq.empty())
        {
            auto curr = pq.top();
            pq.pop();
            if (curr.second == 0 || curr.first <= currDay)
            {
                continue;
            }
            res++;
...
void solve()
{
    ...
    int res = eatenApples(apples, days);
    debug(res);
}
