...
//|> sliding window
int longestSubarrayOP(vector<int> &nums, int limit)
{
    deque<int> minq, maxq;
    int s = 0;
    int e = 0;
    ...
    while (e < n)
    {
        ...
        minq.push_back(e); // Store the index instead of the value

        while (!maxq.empty() && nums[maxq.back()] <= curr)
        {
            maxq.pop_back();
        }
        maxq.push_back(e); // Store the index instead of the value
