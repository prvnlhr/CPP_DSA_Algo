...
vector<vector<string>> suggestedProducts(vector<string> &products, string searchWord)
{
    ...
    for (int i = 0; i < searchWord.size(); i++)
    {
        ...
        auto iter = lower_bound(products.begin(), products.end(), prefix);

        vector<string> suggestions;
        for (int s = iter - products.begin(); s < n && s < iter - products.begin() + 3; s++)
        {
            if (products[s].find(prefix) == 0) //|> find returns index of first occurance, else returns ::npos
            {
                suggestions.push_back(products[s]);
            }
            else
            {
                break;
