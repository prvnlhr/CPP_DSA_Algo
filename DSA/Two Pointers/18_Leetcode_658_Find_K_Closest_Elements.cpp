...
vector<int> findClosestElements(vector<int> &arr, int k, int x)
{
    ...
    {
        int currDist = abs(arr[i] - x);
        pq.push({currDist, arr[i]});
        if (pq.size() > k)
        {
            pq.pop();
        }
...
vector<int> findClosestElementsOP(vector<int> &arr, int k, int x)
{
    int left = 0;
    int right = arr.size() - 1;

    while (right - left + 1 > k) //|> make sure that we maintain a window with at least k elements
    {
        //|> check if element towards left is nearest or element towards right
    ...
    //|> Put all left to right window elements to vector
