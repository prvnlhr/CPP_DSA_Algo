...
vector<int> maxProfit(Job arr[], int n)
{
    ...
    for (int i = 1; i < n; i++)
    {
        maxDeadLine = max(arr[i].deadline, maxDeadLine);
    }
    /*
        > After finding maxDealine, we will create a arr of size maxDeadline+1,
        > to accomodate all deadlines 1 -> maxDeadlines
    */

    //> Intialize slots array with -1
