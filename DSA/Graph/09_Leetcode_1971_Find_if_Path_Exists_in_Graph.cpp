...
bool dfs(vector<int> adjList[], bool visited[], int src, int dest)
{
    visited[src] = true;
    if (src == dest)
    {
    ...
    for (auto adjNode : adjList[src])
    {
        if (!visited[adjNode])
        {
            if (dfs(adjList, visited, adjNode, dest))
            {
                return true;
            }
        }
...
bool validPath(int n, vector<vector<int>> &edges, int source, int destination)
{
    ...
    bool ans = dfs(adjList, visited, source, destination);
    if (ans)
    {
        return true;
    }
...
void solve()
{
    ...
    int src, dest;
    cin >> src >> dest;
    cout << validPath(V, edges, src, dest) << endl;
}
