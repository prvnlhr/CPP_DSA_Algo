...
//|> ---ＳＯＬＶＥ-----------------------------------------------------------------------------------------------------------------------------------------------
int solve(vector<int> &piles, int i, int j, bool isAliceTurn)
{
    if (i == j)
    {
        return isAliceTurn ? piles[i] : -piles[i];
    }

    int maxScore = 0;
    if (isAliceTurn)
    {
        maxScore = max(solve(piles, i + 1, j, false) + piles[i], solve(piles, i, j - 1, false) + piles[j]);
    }
    else
    {
        maxScore = min(-solve(piles, i + 1, j, true) - piles[i], -solve(piles, i, j - 1, true) - piles[j]);
    }
...
bool stoneGameII(vector<int> &piles)
{
    ...
    int aliceScore = solve(piles, 0, piles.size() - 1, true);
