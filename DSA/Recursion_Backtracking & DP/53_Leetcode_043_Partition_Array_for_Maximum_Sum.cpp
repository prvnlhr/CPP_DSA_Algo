...
int recursive(vector<int> &arr, int k, int n, int pos)
{
    if (pos >= n)
    {
        return 0;
    ...
    for (int i = pos; i < min(pos + k, n); i++)
    {
        ...
        sum = max(sum, recursive(arr, k, n, i + 1) + (i - pos + 1) * max_ele);
    }
    return sum;
}

int recursiveMemo(vector<int> &arr, int k, int n, int pos, vector<int> &dp)
{
    if (pos >= n)
    {
        return 0;
    ...
    {
        max_ele = max(max_ele, arr[i]);
        sum = max(sum, recursiveMemo(arr, k, n, i + 1, dp) + (i - pos + 1) * max_ele);
    }
    dp[pos] = sum;
    return sum;
}
...
int maxSumAfterPartitioning(vector<int> &arr, int k)
{
    int n = arr.size();
    vector<int> dp(n + 1, -1);
    return recursiveMemo(arr, k, n, 0, dp);
}
void solve()
{
