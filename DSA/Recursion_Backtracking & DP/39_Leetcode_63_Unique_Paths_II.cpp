...
int helper(vector<vector<int>> &grid, int x, int y, int rows, int cols)
{
    ...
    int a = helper(grid, x + 1, y, rows, cols);
    int b = helper(grid, x, y + 1, rows, cols);
    return a + b;
}
...
int uniquePathsWithObstacles(vector<vector<int>> &obstacleGrid)
{
    int rows = obstacleGrid.size();
    int cols = obstacleGrid[0].size();
    return helper(obstacleGrid, 0, 0, rows, cols);
}
...
void solve()
{
    ...
    auto ans = uniquePathsWithObstacles(obstacleGrid);
    debug(ans);
}
