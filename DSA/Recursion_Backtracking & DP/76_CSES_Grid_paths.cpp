...
int numPath(vector<vector<char>> &grid, int i, int j, int n, vector<vector<int>> &dp)
{
    if (i == n - 1 && j == n - 1)
    {
        return 1;
    ...
    int a = 0;
    int b = 0;
    if (isSafe(i, j + 1, n) && grid[i][j + 1] != '*')
    {
        a = numPath(grid, i, j + 1, n, dp);
    }
    if (isSafe(i + 1, j, n) && grid[i + 1][j] != '*')
    {
        b = numPath(grid, i + 1, j, n, dp);
    }
    dp[i][j] = a + b;
    return a + b;
}
