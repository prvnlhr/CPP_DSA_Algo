...
//> GFG complined solution: needs to output res in increasing ordr
void printNearNodes(TreeNode<int> *root, int low, int high, vector<int> &res)
{
    if (!root)
    {
        return;
...
//>  Generalize solution > does not outputs ans in increasing order
void printNearNodes(TreeNode<int> *root, int low, int high, vector<int> &res)
{
    if (!root)
    {
...
void solve()
{
    ...
    int low = 4;
    int high = 24;
    auto ans = printNearNodes(root, low, high);
    debug(ans);
}
