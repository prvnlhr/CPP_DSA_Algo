...
TreeNode<int> *BSTtoBalBstHelper(vector<int> inorder, int lo, int hi)
{
    if (lo > hi)
    {
        return NULL;
    ...
    root->left = BSTtoBalBstHelper(inorder, lo, mid - 1);
    root->right = BSTtoBalBstHelper(inorder, mid + 1, hi);
    return root;
}
TreeNode<int> *BSTtoBalancedBSt(TreeNode<int> *root)
{
    vector<int> in;
    inorder(root, in);
    debug(in);
    int lo = 0;
    int hi = in.size() - 1;
    TreeNode<int> *newRoot = BSTtoBalBstHelper(in, lo, hi);
    return newRoot;
}

void solve()
{
    vector<int> arr{4, 3, -1, 2, -1, 1, -1, -1, -1};
    TreeNode<int> *root = buildTree(arr);
    TreeNode<int> *newRoot = BSTtoBalancedBSt(root);
    printTree(newRoot);
}
