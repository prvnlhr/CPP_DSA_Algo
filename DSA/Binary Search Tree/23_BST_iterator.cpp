...
public:
    ...
    void putLeftNode(TreeNode<int> *root)
    {
        while (root)
        {
            st1.push_back(root);
    ...
    void putRightNode(TreeNode<int> *root)
    {
        while (root)
        {
            st2.push_back(root);
    ...
    int next()
    {
        ...
        st1.pop_back();
        putLeftNode(temp->right);
        return temp->val;
    }

    bool hasNext()
    {
        return st1.size() == 0 ? false : true;
    }
};

void printST(vector<TreeNode<int> *> st)
{
    for (auto e : st)
    {
        cout << e->val << " ";
