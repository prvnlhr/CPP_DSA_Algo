...
void generatePath(TreeNode<int> *root, int node, vector<int> &res, vector<int> path)
{
    if (!root)
    {
        return;
    ...
    if (root->val < node)
    {
        generatePath(root->right, node, res, path);
    }
    else if (root->val > node)
    {
        generatePath(root->left, node, res, path);
    }
    path.pop_back();
}
vector<int> findRootToNodePath(TreeNode<int> *root, int node)
{
    vector<int> res;
    vector<int> path;
    generatePath(root, node, res, path);
    return res;
}
...
void solve()
{
    ...
    int node = 2;
    auto ans = findRootToNodePath(root, node);
    debug(ans);
}
