...
int findMax(TreeNode<int> *root)
{
    ...
    int lftMax = findMax(root->left);
    int rhtMax = findMax(root->right);

    return max(max(root->val, lftMax), rhtMax);
}
int findMin(TreeNode<int> *root)
{
    ...
    int lftMin = findMin(root->left);
    int rhtMin = findMin(root->right);

    return min(min(root->val, lftMin), rhtMin);
}
void solve()
{
    int ele;
