...
TreeNode<int> *helper1(vector<int> &preorder, int &preIndex, int low, int high, int size)
{
    ...
    //>> 3. Dividing Preorder and repeating
    root->left = helper1(preorder, preIndex, preIndex, nxtGrIndx - 1, size);
    root->right = helper1(preorder, preIndex, nxtGrIndx, high, size);
    return root;
}
...
void solve()
{
    ...
    TreeNode<int> *root = bstFromPreorder(input);
    printInorder(root);
    printTree(root);
}
