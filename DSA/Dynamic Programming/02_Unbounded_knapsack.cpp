...
int knapSackRec(vector<int> weights, vector<int> values, int maxW, int i, int n)
{
    ...
    if (weights[i] <= maxW)
    {
        return max(values[i] + knapSackRec(weights, values, maxW - weights[i], i + 1, n), knapSackRec(weights, values, maxW, i, n));
    }
    else
    {
...
int knapSackRec1(vector<int> weights, vector<int> values, int maxW, int n)
{
    ...
    if (weights[n - 1] <= maxW)
    {
        int ans1 = values[n - 1] + knapSackRec1(weights, values, maxW - weights[n - 1], n);
...
int knapSackMemo(vector<int> weights, vector<int> values, int maxW, int n, vector<vector<int>> DP)
{
    ...
    if (weights[n] <= maxW)
    {
        return DP[n][maxW] = max(values[n] + knapSackMemo(weights, values, maxW - weights[n], n, DP), knapSackMemo(weights, values, maxW, n - 1, DP));
    }
...
int knapSackIter(vector<int> weights, vector<int> values, int maxW)
{
    ...
    for (int i = 1; i < n + 1; i++)
    {
        for (int w = 1; w < maxW + 1; w++)
        {
            ...
            if (weights[i - 1] <= w)
            {
                int a = values[i - 1] + Dp[i][w - weights[i - 1]];
                int b = Dp[i - 1][w];
                Dp[i][w] = max(a, b);
            }
            else
