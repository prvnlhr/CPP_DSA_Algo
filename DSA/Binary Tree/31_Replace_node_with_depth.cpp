...
TreeNode<int> *replaceNodeWithDepthHepler(TreeNode<int> *root, int depth)
{
    ...
    root->val = depth;
    replaceNodeWithDepthHepler(root->left, depth + 1);
    replaceNodeWithDepthHepler(root->right, depth + 1);

    return root;
}

TreeNode<int> *replaceNodeWithDepth(TreeNode<int> *root)
{

    return replaceNodeWithDepthHepler(root, 0);
}
...
void solve()
{
    ...
    auto newRoot = replaceNodeWithDepth(root);
    printTree(newRoot);
}
