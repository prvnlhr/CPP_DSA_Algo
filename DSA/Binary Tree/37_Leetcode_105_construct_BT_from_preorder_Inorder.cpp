...
TreeNode<int> *buildBTTree(vector<int> &inorder, vector<int> &preorder, int left, int right)
{
    ...
    TreeNode<int> *root = new TreeNode(preorder[rootIndex]);
    rootIndex++;

    root->left = buildBTTree(inorder, preorder, left, mpp[root->val] - 1);
    root->right = buildBTTree(inorder, preorder, mpp[root->val] + 1, right);
    return root;
}
...
TreeNode<int> *buildTree(vector<int> &preorder, vector<int> &inorder)
{
    ...
    return buildBTTree(inorder, preorder, 0, preorder.size() - 1);
}
