...
void generatePaths(TreeNode<int> *root, vector<int> path, vector<vector<int>> &allPaths)
{
    ...
    path.push_back(root->val);
    generatePaths(root->left, path, allPaths);
    generatePaths(root->right, path, allPaths);
    path.pop_back();
}

void generatePaths1(TreeNode<int> *root, string path, vector<string> &allPaths)
{
    if (!root)
    {
        return;
    ...
    path = path + to_string(root->val) + "->";
    generatePaths1(root->left, path, allPaths);
    generatePaths1(root->right, path, allPaths);
    path.pop_back();
}
...
vector<string> binaryTreePaths(TreeNode<int> *root)
{
    ...
    generatePaths1(root, path, allPaths);
    debug(allPaths);
    return allPaths;
}
