...
TreeNode<int> *buildBTTree(vector<int> &inorder, vector<int> &postorder, int left, int right, int &rootIndex)
{
    ...
    root->right = buildBTTree(inorder, postorder, left, indx - 1, rootIndex);
    root->left = buildBTTree(inorder, postorder, indx + 1, right, rootIndex);
    return root;
}

TreeNode<int> *buildTree(vector<int> &postorder, vector<int> &inorder)
{
    mpp.clear();
    ...
    return buildBTTree(inorder, postorder, 0, postorder.size() - 1, rootIndex);
}
