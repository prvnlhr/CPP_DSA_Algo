...
//[[5,4,11,2],[5,8,4,5]]
void getPaths(TreeNode<int> *root, int targetSum, int currSum, vector<vector<int>> &res, vector<int> path)
{
    ...
    getPaths(root->left, targetSum, currSum, res, path);
    getPaths(root->right, targetSum, currSum, res, path);
    path.pop_back();
}
...
vector<vector<int>> pathSum(TreeNode<int> *root, int targetSum)
{
    ...
    vector<int> path;
    getPaths(root, targetSum, 0, res, path);
    return res;
}
