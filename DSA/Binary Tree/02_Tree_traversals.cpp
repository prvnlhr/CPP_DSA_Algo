...
void helper(TreeNode<int> *root, vector<int> &res)
{
    if (!root)
    {
        return;
...
vector<int> inorderLeetcode(TreeNode<int> *root)
{
    vector<int> res;
    helper(root, res);
    return res;
...
void levelHelper(TreeNode<int> *root, vector<int> &res)
{
    queue<TreeNode<int> *> q;
...
{
    vector<int> res;
    levelHelper(root, res);
    return res;
}
void solve()
{

    //> All traversal : pre, in, post, level, boundary
    int ele;
    vector<int> input;
    ...
    vector<int> op = inorderLeetcode(root);
    for (auto node : op)
    {
        cout << node << " ";
    }
    cout << endl;
    auto levelOrderTr = levelOrder(root);
    debug(levelOrderTr);
    // preorder(root);
    // cout << endl;
