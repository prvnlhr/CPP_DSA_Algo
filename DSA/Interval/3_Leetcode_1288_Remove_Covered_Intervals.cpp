...
int removeCoveredIntervals(vector<vector<int>> &intervals)
{
    ...
    for (int i = 1; i < intervals.size(); i++)
    {
        ...
        int stack_s = st.back()[0];
        int stack_e = st.back()[1];

        if ((stack_s <= curr_s && stack_e >= curr_e) || (stack_s >= curr_s && stack_e <= curr_e))
        {
            st.pop_back();
            st.push_back({min(stack_s, curr_s), max(stack_e, curr_e)});
        }
        else
        {
            st.push_back({curr_s, curr_e});
        }
    }
    return st.size();
}
// Lee's solution -> instead of stack, we use two variables left and right
...
void solve()
{
    ...
    // vector<vector<int>> intervals{{1, 2}, {2, 5}, {3, 4}, {1, 10}, {3, 6}};
