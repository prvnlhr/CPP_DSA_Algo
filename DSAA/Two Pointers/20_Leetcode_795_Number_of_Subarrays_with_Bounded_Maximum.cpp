...
int numSubarrayBoundedMax(vector<int> &nums, int left, int right)
{
    ...
    while (win_end < n)
    {
        ...
        int curr_ele = nums[win_end];

        if (left <= curr_ele && curr_ele <= right) //|>  --> [left,right] <--
        {
            prev_count = win_end - win_start + 1;
            res += prev_count;
        }
        else if (curr_ele < left) //|>   <--[left,right]
        {
            res += prev_count;
        }
        else //|> out of range of <--[left,right]-->
        {
            win_start = win_end + 1;
            prev_count = 0;
        }
