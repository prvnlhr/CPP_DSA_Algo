...
vector<int> partitionLabels(string s)
{
    ...
    int j = 0;
    vector<int> res;
    int rangeStart = mpp[s[0]].first;
    int rangeEnd = mpp[s[0]].second;
    j = 1;
    ...
    {
        char currChar = s[j];
        auto [start, end] = mpp[currChar];

        if (start <= rangeEnd)
        {
            rangeStart = min(rangeStart, start);
            rangeEnd = max(rangeEnd, end);
        }
        else if (start > rangeEnd)
        {
            res.push_back(abs(rangeEnd - rangeStart) + 1);
            rangeStart = start;
            rangeEnd = end;
        }
