...
int findPairs(vector<int> &nums, int k)
{
    ...
    while (i < nums.size())
    {
        ...
        while (j < nums.size())
        {
            int diff = abs(currVal - nums[j]);
            if (diff == k && st.find({currVal, nums[j]}) == st.end() && st.find({nums[j], currVal}) == st.end())
            {
                st.insert({currVal, nums[j]});
                cntPairs++;
            }
            j++;
        }
...
// |>O(N)
int findPairsOP(vector<int> &nums, int k)
{
