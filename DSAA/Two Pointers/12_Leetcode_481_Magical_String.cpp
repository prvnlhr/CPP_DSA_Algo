...
int magicalString(int n)
{
    ...
    {
        char nextChar = (magicStr.back() == '1' ? '2' : '1');
        char timesToAppendNextChar = magicStr[index] - '0';

        for (int i = 0; i < timesToAppendNextChar; i++)
        {
            magicStr += nextChar;
            if (nextChar == '1' && magicStr.size() <= n) //|>  if string of n size already formed,
                                                         //|>  then nextChar will not be append, so no need to count 1
            {
                onesCount++;
            }
        }
