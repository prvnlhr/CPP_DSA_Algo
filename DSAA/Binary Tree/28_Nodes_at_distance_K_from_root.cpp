...
void nodeAtKDisHepler(TreeNode<int> *root, int k, vector<int> &nodes)
{
    if (!root)
    {
        return;
    ...
    nodeAtKDisHepler(root->left, k - 1, nodes);
    nodeAtKDisHepler(root->right, k - 1, nodes);
}

vector<int> nodeAtKDis(TreeNode<int> *root, int k)
{
    vector<int> nodes;
    nodeAtKDisHepler(root, k, nodes);
    debug(nodes);
    return nodes;
}
