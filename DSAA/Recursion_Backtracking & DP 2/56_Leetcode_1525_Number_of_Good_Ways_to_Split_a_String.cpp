...
int numSplits(string s)
{
    ...
    set<char> st;
    vector<pair<int, int>> indexVec(s.size());

    for (int i = 0; i < s.size(); i++)
    {
        int ch = s[i];
        st.insert(ch);
        indexVec[i].first = st.size();
    }
    ...
    for (int i = s.size() - 1; i >= 0; i--)
    {
        int ch = s[i];
        st.insert(ch);
        indexVec[i].second = st.size();
    }
    int res = 0;
    for (int i = 0; i < s.size() - 1; i++)
    {
        int leftUniqueCnt = indexVec[i].first;
        int rightUniqueCnt = indexVec[i + 1].second;
        if (leftUniqueCnt == rightUniqueCnt)
        {
            res++;
        }
    }
