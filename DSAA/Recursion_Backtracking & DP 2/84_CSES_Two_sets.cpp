...
const int MAX_SUM = 127265;
const int MAX_N = 505;
int dp[MAX_N][MAX_SUM];

int recursiveDP(int n, int sum, int curr, int reqSum)
{
    ...
    int take = recursiveDP(n, sum + curr, curr + 1, reqSum);
    int not_take = recursiveDP(n, sum, curr + 1, reqSum);

    dp[curr][sum] = (take + not_take) % MOD;
    return dp[curr][sum];
}
...
void solve()
{
    ...
    else
    {
        ...
        int ans = recursiveDP(n, 0, 1, sum / 2) % MOD;
        cout << ans << endl;
    }
}
