...
void solve()
{
    ...
    for (int i = 0; i < r; i++)
    {
        for (int j = 0; j < c; j++)
        {
            ...
            for (int k = 0; k < 8; k++)
            {
                ...
                if (isSafe(x, y, r, c))
                {
                    //> yaha garbad hai ?? sotring 2 bit ,so cant check diectly 1 or 0, need do & 1;

                    //> if(board[x][y] 1) -> '''cnt do this'''
                    //>
                    if (board[x][y] & 1)
                    {
                        cnt++;
    ...
    for (int i = 0; i < r; i++)
    {
        for (int j = 0; j < c; j++)
        {
            ...
            //> right shifting to get new state stored at leftmost position
            board[i][j] >>= 1;
        }
    }
    debug(board);
